    ...
    <Carousel :value="filteredProducts" :numVisible="5" :numScroll="1" class="mt-4" :showIndicators="false"
      ...
      <template #item="slotProps">
        <div class="border-r border-gray-200 rounded-lg p-4 text-center bg-white shadow-md relative group">
          ...
          <!-- Product Image -->
          <img :src="slotProps.data.image" class="h-32 mx-auto mb-2 transition-transform group-hover:scale-105"
            alt="Product Image" />
          ...
          <div class="flex flex-col items-center text-center p-2">
            ...
            <p class="text-red-500 font-semibold">
              ...
              <span class="line-through text-gray-400">{{ formattedPrice(slotProps.data.oldPrice) }}</span>
            </p>
          </div>
...
const productsWithRecent = computed(() => {
  return store.products.flatMap((product) =>
    product.models.map((model) => ({
      ...
      image: model.images.find((img) => img.isPrimary)?.uploadUrl || "default-image.jpg",
    }))
  );
});
...
const filteredProducts = computed(() =>
  productsWithRecent.value.filter(
    (product) =>
      product.subCategory.category.name === selectedTab.value || selectedTab.value === "Recent"
  )
);
</script>
